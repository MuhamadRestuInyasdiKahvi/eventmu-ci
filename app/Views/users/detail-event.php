...
<body class="">
    ...
    <!-- Event  -->
    <section class="bg-gradient-to-b from-black-primary to-purple-3 pb-10" id="event">
        <div class='max-w-7xl mx-auto'>
            <div class='flex flex-col lg:flex-row lg:items-center mb-2 gap-y-5 lg:gap-y-0 lg:justify-between py-5'>
                <a href="javascript:window.history.go(-1)" class="mx-5 text-white w-full lg:w-2/12 "> < </a>
                <div class="w-full lg:w-10/12 flex flex-col lg:flex-row justify-end items-center gap-x-5">
                    <span class='w-11/12  lg:w-4/12  h-1 bg-gradient-to-l from-purple-1 to-purple-2 rounded-md lg:mx-2 my-2'></span>
                    <h3 class='font-semibold text-lg lg:text-3xl text-white font-sans'><?= $event->judul ?></h3>
                </div>
            </div>
            <div class="flex-col gap-y-3 flex mx-5 lg:mx-0">
                <div class='lg:w-full lg:h-[500px]'>
                    <img src="<?= base_url('template/assets/img/img-event/' . $event->img_event) ?>" class="object-fill w-full h-full ">
                </div>
                <div class="text-gray-200 text-base lg:text-xl font-sans"><?= $event->deskripsi ?></div>
                <p class='font-sans text-sm font-medium text-white'><?= $event->penyelenggara ?> | <?= date('d M y', strtotime($event->start_event)) ?></p>
            </div>
        </div>
    </section>
